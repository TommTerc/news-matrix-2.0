...
class EngagementService {
   ...
   * Remove article like
   */
  async unlikeArticle(articleUrl: string): Promise<boolean> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return false;
   ...
   */
  async hasLiked(articleUrl: string): Promise<boolean> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return false;
   ...
   */
  async getLikeCount(articleUrl: string): Promise<number> {
   ...
   * Track article share
   */
  async shareArticle(articleUrl: string, platform: string = 'internal'): Promise<boolean> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return false;
   ...
   */
  async getShareCount(articleUrl: string): Promise<number> {
    const { count, error } = await supabase
      .from('article_shares')
      .select('*', { count: 'exact', head: true })
      .eq('article_url', articleUrl);
   ...
   */
  async getAverageReadTime(articleUrl: string): Promise<number> {
